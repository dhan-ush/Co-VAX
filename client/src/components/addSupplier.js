...
function AddSupplier(props) {
  ...
  const supplierSubmit = (e) => {
    e.preventDefault();
    const regexx = /\d+/;
    if (!regexx.test(supplier.jabs_available)) {
      alert("Please Enter Valid jabs !!!");
      return;
    }else{
    alert("Supplier Added Successfully");
    axios
      .post("http://localhost:3001/AddSupplier", supplier)
      .then((response) => {
              ...
              >
                <div className={st.detailsBox}>
                  <div className={st.rightL}>
                      ...
                      <div className={st.radioRow}>
                        <div className={st.op}>
                          <input
                            ...
                            value="covishield"
                          />
                          <label className={st.radiolabel} for="covishield">
                            Covishield
                          </label>
                          <br></br>
                        </div>
                        <div className={st.op}>
                          <input
                            ...
                            value="covaxin"
                          />
                          <label className={st.radiolabel} for="covaxin">
                            Covaxin
                          </label>
                          <br></br>
                        </div>
                    ...
                    <div>
                      <label className={st.label} htmlFor="jabs_available">
                        Jabs Available :{" "}
                      </label>
                      <br />
                      <input
